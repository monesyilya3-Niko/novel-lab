...
> 生成日期：2026-09-11 ｜ 项目已从另一套 WorkBuddy 实例迁移至 niko 工作区
...
### 运行方式
```bash
python run_tests.py          # 全量（基线 272 用例）
```
...
- 当前实测口径：资产 **55** / 报告 **8** / 书 **6**（已拆 4）/ 题材 **33** / 测试 **272**
- ⚠️ 历史文档中的「17 资产 / 33 测试 / 3 本书 / 68 资产」等均为**过时或错误口径**
...
| `RULES.md` | 可执行规则全集（15 章，含拆书实操经验） | ✅ 大体可用（§二为历史存档） |
| `AGENTS.md` | 本文件 | ✅ 权威 |
| `PROJECT_SUMMARY.md` | 项目总结 | ❌ **严重过时**（称 17 资产 / 33 测试），待重写 |
| `HANDOFF.md` | 交接文档 | ❌ **过时**，待重写 |
| `docs/` | PRD / DESIGN / 架构 / 复盘 | ⚠️ 混合，注意日期 |
